...
from .models import News, Content, Image, Quote, CodeBlock, Video, Ad, List, Title, Subtitle, BlogContent
...
def blogsupdate(request, blog_id):
    try:
        blogiop = get_object_or_404(News, id=blog_id)
...
def image_add(request, blog_id):
    ...
    try:
        blogg = News.objects.get(id = blog_id)
...
def remove_blog(request, content_id):
    try:
        blog_content = get_object_or_404(News, id=content_id)
        blog_content.delete()
...
def blog_detail_app(request, blog_id):
    blog = get_object_or_404(News, id=blog_id)
...
def create_blog(request):
    ...
    print(dicrip)
    print(auto)
    blo = News(title=title, description = dicrip, author=auto)
    blo.save()
...
def blog_detail(request, blog_id):
    ...
    blog = News.objects.get(id=blog_id)
