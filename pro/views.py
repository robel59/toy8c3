...
from .models import Project, Content, Image, Quote, CodeBlock, Video, Ad, List, Title, Subtitle, BlogContent
...
def blogsupdate(request, blog_id):
    try:
        blogiop = get_object_or_404(Project, id=blog_id)
...
def image_add(request, blog_id):
    ...
    try:
        blogg = Project.objects.get(id = blog_id)
...
def remove_blog(request, content_id):
    try:
        blog_content = get_object_or_404(Project, id=content_id)
        blog_content.delete()
...
def create_blog(request):
    ...
    print(dicrip)
    print(auto)
    blo = Project(title=title, description = dicrip, author=auto)
    blo.save()
...
def blog_detail(request, blog_id):
    ...
    blog = Project.objects.get(id=blog_id)
