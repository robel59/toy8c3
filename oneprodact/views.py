...
from .models import Service, Content, Image, Quote, CodeBlock,Order, Video, Ad, List, Title, Subtitle, BlogContent
from .serializers import BlogSerializer12, OrderSerializer, ContentSerializer, ImageSerializer, QuoteSerializer, CodeBlockSerializer, VideoSerializer, AdSerializer, ListSerializer, TitleSerializer, SubtitleSerializer
...
def blogsupdate(request, blog_id):
    try:
        blogiop = get_object_or_404(Service, id=blog_id)
...
def image_add(request, blog_id):
    ...
    try:
        blogg = Service.objects.get(id = blog_id)
        image = request.FILES.get('image')
        blogg.image=image
        blogg.save()
        serializer = BlogSerializer12(blogg)
...
def remove_blog(request, content_id):
    try:
        blog_content = get_object_or_404(Service, id=content_id)
        blog_content.delete()
...
def blog_detail_app(request, blog_id):
    blog = get_object_or_404(Service, id=blog_id)
    serializer = BlogSerializer12(blog)
...
def create_blog(request):
    ...
    print(dicrip)
    print(auto)
    blo = Service(title=title, description = dicrip, author=auto)
    blo.save()

    # Serialize the client data to return in the response
    serializer = BlogSerializer12(blo)

    return JsonResponse(serializer.data, status=201)
'''
    print('ffffffffdddddsssddd')
    serializer = BlogSerializer12(data=request.data)
...
def listblog(request):
    ...
    serializer_class = BlogSerializer12(queryset, many=True)
...
def blog_detail(request, blog_id):
    ...
    blog = Service.objects.get(id=blog_id)
