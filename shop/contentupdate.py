...
from .models import Item, Content, Imagec, Quote, CodeBlock, Video, Ad, List, Title, Subtitle, BlogContent
...
def blogsupdate(request, blog_id):
    try:
        blogiop = get_object_or_404(Item, id=blog_id)
...
def blog_detail_app(request, blog_id):
    blog = get_object_or_404(Item, id=blog_id)
...
def add_content(request, blog_id):
    if request.method == 'POST':
        ...
        if content_type == 'text':
            ...
            content = Imagec.objects.create(blog=blog, info=content_data['info'], image=image)
            content_type_obj = ContentType.objects.get_for_model(Imagec)
...
def create_galry(request, blog_id):
    ...
    print(info)
    image = request.FILES.get('image')
    content = Imagec.objects.create(blog=blog, info=info, image=image)
    content_type_obj = ContentType.objects.get_for_model(Imagec)
    BlogContent.objects.create(
            blog=blog,
            content_type=content_type_obj,
...
def blog_detail(request, blog_id):
    ...
    blog = Item.objects.get(id=blog_id)
